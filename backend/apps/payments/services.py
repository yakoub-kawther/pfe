...
def confirm_payment(payment_id: int) -> Payment:
    ...
    inscription.status = Inscription.Status.CONFIRMED   # adjust field name if different
...
def get_student_payments(student_id: int) -> list:
    payments = (
        Payment.objects
        .filter(inscription__student_id=student_id)
...
def _send_payment_confirmation_notification(payment: Payment) -> None:
    """
    Placeholder for the real notification logic.
    Replace with your actual notification service (email, SMS, push…).
    """
