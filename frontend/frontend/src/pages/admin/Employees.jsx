      ...
      <div className="max-w-6xl mx-auto flex flex-col gap-6 pt-6">
        ...
        <div className="flex items-center justify-between" style={{ marginTop: "30px" }}>
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2 rounded-lg  bg-[#701366] text-white text-sm hover:bg-white hover:text-[#701366] border border-[#701366] transition-all"
          >
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-[#F8E0F8] h-12 text-[#701366] text-left">
                <th className="py-3 text-sm" style={{ paddingLeft: "30px" }}>Name</th>
                <th className="px-4 py-3 text-sm">Phone</th>
                <th className="px-4 py-3 text-sm">Position</th>
                <th className="px-4 py-3 text-sm">Hire Date</th>
            ...
            <tbody className="divide-y divide-[#f8e0f8]">
              {employeesData.map((emp) => (
                <tr key={emp.id} className="hover:bg-[#fffafe] transition-colors duration-100 h-12">
                  <td className="py-3 text-[#701366]" style={{ paddingLeft: "30px" }}>{emp.name}</td>
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-Inter ${statusStyles[emp.status]}`}>
                      ● {emp.status}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <button
                        aria-label="Edit"
                        onClick={() => navigate("/Edit_employee", { state: { employee: emp } })}
                      ...
                      <button
                        aria-label="More"
                        onClick={() => navigate("/Employee_profile", { state: { employee: emp } })}
                        className="p-1.5 rounded-sm text-[#701366] hover:text-white hover:bg-[#701366] transition-all hover:scale-110"
                      >
