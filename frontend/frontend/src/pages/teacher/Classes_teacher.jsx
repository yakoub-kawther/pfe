...
export default function Classes_teacher() {
  ...
  useEffect(() => {
    // Step 1: get logged-in account to find teacher person_id
    apiFetch("/account/me/")
      .then(r => r.json())
      .then(account => {
        console.log("account/me/:", account);
        const teacherId = account?.person_id ?? null;
        if (!teacherId) throw new Error("Could not resolve teacher ID from account.");
        // Step 2: fetch classes assigned to this teacher
  ...
  const filtered = classes.filter(c => {
    ...
    return (
      ...
      (c.language?.language_name ?? "").toLowerCase().includes(q)
    );
  });
    ...
    <Teacher_layout>
      <div style={{ maxWidth: "1100px", margin: "40px auto", fontFamily: "Inter, sans-serif", padding: "0 24px", display: "flex", flexDirection: "column", gap: "24px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "16px", flexWrap: "wrap" }}>
          <div>
            <h2 style={{ fontSize: "24px", color: "#701366", fontFamily: "Inter, sans-serif", margin: 0 }}>My Classes</h2>
            <p style={{ fontSize: "13px", color: "#b48ab0", margin: "4px 0 0" }}>{filtered.length} classes assigned to you</p>
          </div>
        ...
        {/* Error */}
        {error && (
          <div style={{ background: "#fee2e2", color: "#dc2626", padding: "12px 16px", borderRadius: "10px", fontSize: "13px" }}>
            {error}
          </div>
        )}
              ...
              <tr style={{ background: "linear-gradient(90deg, #f8e0f8 0%, #fdf4fd 100%)", height: "52px" }}>
                {[
                  ...
                  <th key={label} style={{
                    ...
                    textAlign: "left", color: "#701366", letterSpacing: "0.03em", textTransform: "uppercase",
                  }}>{label}</th>
                ))}
              </tr>
            </thead>
                  ...
                  >
                    ...
                    <td style={{ padding: "0 16px" }}>
                      <span style={{
                        ...
                        padding: "3px 12px", borderRadius: "9999px", fontSize: "12px", fontWeight: 600,
