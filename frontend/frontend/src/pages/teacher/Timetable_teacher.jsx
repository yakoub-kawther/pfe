...
function weekStart(date) { const d=new Date(date); const day=d.getDay(); d.setDate(d.getDate()+(day===0?-6:1-day)); d.setHours(0,0,0,0); return d; }
...
function getEventLayout(dayEvs) {
  ...
  const colEnds=[]; const colAssign=new Map();
  for (const ev of sorted) {
    let placed=false;
    for (let i=0;i<colEnds.length;i++) {
      if (colEnds[i]<=ev.startHour) { colAssign.set(ev.id,i); colEnds[i]=ev.startHour+ev.duration; placed=true; break; }
    }
    if (!placed) { colAssign.set(ev.id,colEnds.length); colEnds.push(ev.startHour+ev.duration); }
  }
...
function EventBlock({ ev, style, size="sm", onClick }) {
  ...
  return (
    <div onClick={onClick} style={{ position:"absolute", ...style, background:c.bg, border:`1.5px solid ${c.border}`, borderRadius:size==="lg"?14:9, padding:size==="lg"?"10px 14px":"5px 7px", overflow:"hidden", cursor:"pointer", transition:"box-shadow 0.15s, transform 0.15s" }}
...
function EventPopup({ ev, anchorRect, onClose }) {
  ...
  const c = DAY_COLORS[ev.dow];
  return (
    <div ref={popupRef} style={{ position:"fixed", top:anchorRect?Math.min(anchorRect.bottom+8,window.innerHeight-160):"50%", left:anchorRect?Math.min(anchorRect.left,window.innerWidth-230):"50%", zIndex:200, background:"white", borderRadius:14, boxShadow:"0 8px 32px rgba(112,19,102,0.18)", padding:"16px", width:220, border:`2px solid ${c.border}33` }}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        <div>
          ...
          {ev.room&&<div style={{fontSize:11,color:"#999",marginTop:2}}>📍 {ev.room}</div>}
        </div>
  ...
  return (
    <div style={{display:"flex",overflowX:"auto"}}>
      ...
      {Array.from({length:7},(_,di)=>{
        const date=addDays(ws,di); const isToday=isSameDay(date,today);
        const dayEvs=events.filter(e=>e.dow===di); const layout=getEventLayout(dayEvs);
        return (
          <div key={di} style={{flex:1,minWidth:90,position:"relative"}}>
            ...
            {dayEvs.map(ev=>{
              const {col,totalCols}=layout.get(ev.id); const w=100/totalCols;
...
function DayView({ cursor, events, today, onEventClick }) {
  const di=dowIndex(cursor); const isToday=isSameDay(cursor,today);
  const dayEvs=events.filter(e=>e.dow===di); const layout=getEventLayout(dayEvs);
  return (
  ...
  return (
    <div style={{display:"grid",gridTemplateColumns:"repeat(7, 1fr)",gap:1,background:"#f0eaf8",borderRadius:10,overflow:"hidden"}}>
      ...
      {Array.from({length:totalCells},(_,i)=>{
        ...
        const cellDate=new Date(year,month,dayNum); const di=(cellDate.getDay()+6)%7;
        const isToday=isSameDay(cellDate,today); const c=DAY_COLORS[di];
...
export default function Timetable_teacher() {
  ...
  // Fetch schedule when class selected
  useEffect(() => {
  if (!selectedClassId) return;

  const fetchSchedule = async () => {
    setLoadingSchedule(true);  // ← inside async fn, not directly in effect
    try {
      const r    = await apiFetch(`/academic/schedules/?class_obj=${selectedClassId}`);
  ...
  const getNavLabel = () => {
    if (view==="Week") { const ws=weekStart(cursor),we=addDays(ws,6); return `${ws.getDate()} ${MONTHS[ws.getMonth()].slice(0,3)} – ${we.getDate()} ${MONTHS[we.getMonth()].slice(0,3)} ${we.getFullYear()}`; }
  ...
  return (
    <Teacher_layout>
      <div style={{ maxWidth:"1100px", margin:"40px auto", fontFamily:"Inter, sans-serif", padding:"0 24px", display:"flex", flexDirection:"column", gap:"24px" }}>
        ...
        <h2 style={{ fontSize:"24px", color:"#701366", margin:0 }}>Time Table</h2>

        {/* Class selector */}
        <div>
          <p style={{ fontSize:13, color:"#b48ab0", marginBottom:12 }}>Select a class to view its timetable</p>
          {loadingClasses ? (
            <p style={{ color:"#b48ab0", fontSize:13 }}>Loading classes...</p>
          ) : (
            <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
              {classes.map(cls => {
                ...
                return (
                  <div key={cls.id} onClick={() => setSelectedClassId(cls.id)} style={{ padding:"10px 20px", borderRadius:12, cursor:"pointer", border:isSelected?"2px solid #701366":"2px solid #f0d8ee", background:isSelected?"#701366":"white", boxShadow:isSelected?"0 4px 14px rgba(112,19,102,0.2)":"0 1px 4px rgba(112,19,102,0.06)", transition:"all 0.15s", display:"flex", alignItems:"center", gap:10 }}>
                    <div>
                      <div style={{ fontSize:14, fontWeight:600, color:isSelected?"white":"#701366" }}>{cls.name}</div>
                      <div style={{ fontSize:11, color:isSelected?"#f0d8ee":"#b48ab0", marginTop:2 }}>{cls.language?.language_name}</div>
                    </div>
                    <span style={{ padding:"3px 10px", borderRadius:9999, fontSize:11, fontWeight:600, background:isSelected?"rgba(255,255,255,0.2)":(levelColors[levelName]||"#f3f4f6"), color:isSelected?"white":(levelText[levelName]||"#374151") }}>
                      {levelName || "—"}
                    </span>
                  </div>
                );
          ...
          <>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:10 }}>
              <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>
                <button onClick={() => setCursor(new Date(today))} style={{ fontSize:"0.78rem", color:"#701366", cursor:"pointer", border:"1.5px solid #e0d6f0", background:"white", borderRadius:8, padding:"3px 10px", fontWeight:400 }}>Today</button>
            ...
            <div style={{ background:"white", borderRadius:16, boxShadow:"0 2px 16px rgba(112,19,102,0.07)", padding:16, overflow:"hidden" }}>
              {loadingSchedule ? (
                <div style={{ textAlign:"center", padding:"60px", color:"#b48ab0", fontSize:14 }}>Loading schedule...</div>
              ) : (
                <>
        ...
        {popup && <EventPopup ev={popup.ev} anchorRect={popup.rect} onClose={() => setPopup(null)}/>}
