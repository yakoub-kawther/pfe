...
// ── Convert schedule from API to event format ──────────────
function scheduleToEvent(schedule, cls) {
  const dow       = DOW_MAP[schedule.day_of_week?.toLowerCase()] ?? 0;
...
export default function Time_table_student() {
  ...
  useEffect(() => {
    const fetchClasses = async () => {
      ...
      try {
        ...
        const sid    = meData.person_id;

        const res  = await apiFetch(`/inscriptions/student/${sid}/current/`);
  ...
  // ── Fetch schedules when class selected ──────────────────
  useEffect(() => {
    ...
    const fetchSchedules = async () => {
      ...
      try {
        const res   = await apiFetch(`/academic/schedules/?class_obj=${selectedClassId}`);
        const data  = await res.json();
        const list  = Array.isArray(data) ? data : (data.results ?? []);
        const cls   = classes.find(c => c.id === selectedClassId);
        // ✅ convert schedule → event format
        setEvents(list.map(s => scheduleToEvent(s, cls)));
      } catch {}
                ...
                return (
                  <div key={cls.id} onClick={() => setSelectedClassId(cls.id)} style={{ padding:"10px 20px", borderRadius:12, cursor:"pointer", border:isSelected?"2px solid #701366":"2px solid #f0d8ee", background:isSelected?"#701366":"white", boxShadow:isSelected?"0 4px 14px rgba(112,19,102,0.2)":"0 1px 4px rgba(112,19,102,0.06)", transition:"all 0.15s", display:"flex", alignItems:"center", gap:10 }}>
                    <div>
                      ...
                      <div style={{ fontSize:11, color:isSelected?"#f0d8ee":"#b48ab0", marginTop:2 }}>{cls.language}</div>
                    </div>
                    <span style={{ padding:"3px 10px", borderRadius:9999, fontSize:11, fontWeight:600, background:isSelected?"rgba(255,255,255,0.2)":(levelColors[lvl]||"#f3f4f6"), color:isSelected?"white":(levelText[lvl]||"#374151") }}>
                      {lvl}
                    </span>
                  </div>
                );
          ...
          <>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16, flexWrap:"wrap", gap:10 }}>
              <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>
                ...
                <button onClick={() => navCalendar(-1)} style={{ border:"1.5px solid #e0d6f0", background:"white", borderRadius:8, padding:"2px 10px", fontSize:"1rem", color:"#701366", cursor:"pointer" }}>‹</button>
