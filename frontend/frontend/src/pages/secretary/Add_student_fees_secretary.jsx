...
export default function AddStudentPayment() {
  ...
  const handleSubmit = () => {
    ...
    setSubmitted(true);
    setTimeout(() => navigate("/Fees_secretary"), 1400);
  };
  ...
  const levels = ["Beginner A1","Beginner A2","Pre-Intermediate B1","Intermediate B2","Advanced C1","Advanced C2"];
      ...
      <div style={{ width: "100%", maxWidth: "680px", margin: "30px auto 0", boxSizing: "border-box", paddingBottom: "40px" }}>
        ...
        <div style={{ background: "white", borderRadius: "20px", boxShadow: "0 2px 16px rgba(112,19,102,.08)", overflow: "hidden" }}>
          ...
          <div style={{ background: "linear-gradient(135deg,#8a1a7e 0%,#701366 100%)", padding: "28px 32px", display: "flex", alignItems: "center", gap: "14px" }}>
            ...
            <div>
              <h2 style={{ fontFamily: F, fontSize: "20px", fontWeight: 600, color: "white", margin: 0 }}>Add Student Payment</h2>
              <p style={{ fontFamily: F, fontSize: "13px", color: "rgba(255,255,255,.65)", margin: "3px 0 0" }}>Record a new payment for a student</p>
            </div>
          </div>
          ...
          <div style={{ padding: "32px", display: "flex", flexDirection: "column", gap: "20px" }}>
            ...
            {/* Row 1: Student Name + Level */}
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }}>
              <Field label="Student Name" required>
                <TextInput value={form.studentName} onChange={set("studentName")} placeholder="Full name" />
                {errors.studentName && <p style={{ color: "#dc2626", fontSize: "12px", margin: "2px 0 0", fontFamily: F }}>{errors.studentName}</p>}
              </Field>
              <Field label="Level" required>
                ...
                {errors.level && <p style={{ color: "#dc2626", fontSize: "12px", margin: "2px 0 0", fontFamily: F }}>{errors.level}</p>}
              </Field>
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }}>
              ...
              <Field label="Amount (DA)" required>
                <TextInput value={form.amount} onChange={set("amount")} placeholder="e.g. 3,500" />
                {errors.amount && <p style={{ color: "#dc2626", fontSize: "12px", margin: "2px 0 0", fontFamily: F }}>{errors.amount}</p>}
              </Field>
            </div>
            ...
            <Field label="Payment Status" required>
              <div style={{ display: "flex", gap: "10px" }}>
                {["Paid", "Unpaid", "Partial"].map((s) => {
                  ...
                  const colors = { Paid: ["#dcfce7","#16a34a"], Unpaid: ["#fee2e2","#dc2626"], Partial: ["#fff7ed","#ea580c"] };
                  return (
                    <button
                      key={s} type="button"
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: "12px" }}>
              <button
                onClick={() => navigate("/Fees_secretary")}
