      ...
      <table className="w-full text-sm" id="studentsTable">
        <thead>
          <tr className="bg-[#F8E0F8]  h-12.5  text-[#701366] font-Inter text-left  ">
           <th className="pr-4 py-3 text-sm font-Inter" style={{ paddingLeft: "30px" }}>Name</th>            <th className="px-4 py-3 text-sm font-Inter">Contact</th>
        ...
        <tbody className="divide-y  divide-[#f8e0f8]">
          {studentsData.map((student, idx) => (
            ...
            >
              <td className="pr-4 py-3 font-Inter text-[#701366]" style={{ paddingLeft: "30px" }}>{student.name}</td>              <td className="px-4 py-3 text-[#701366]">{student.contact}</td>
              <td className="px-4 py-3 text-[#701366]">{student.parent}</td>
              <td className="px-4 py-3">
                <span
                  className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium ${statusStyles[student.status.color]}`}
                >
                  ● {student.status.text}
                </span>
              </td>
              <td className={`px-4 py-3 font-Inter ${student.status.color === "green" ? "text-green-600" : student.status.color === "red" ? "text-red-500" : "text-yellow-500"}`}>
                {student.attendance}
              </td>
